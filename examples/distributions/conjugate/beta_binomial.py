...
from probability.distributions.conjugate.beta_binomial_conjugate import CBetaBinomial
...
def plot_wikipedia_pmfs():
    ...
    CBetaBinomial(alpha=0.2, beta=0.25, n=10, m=1).plot(k=k, color='black', ax=ax, **kwargs)
    CBetaBinomial(alpha=0.7, beta=2, n=10, m=1).plot(k=k, color='red', ax=ax, **kwargs)
    CBetaBinomial(alpha=2, beta=2, n=10, m=1).plot(k=k, color='green', ax=ax, **kwargs)
    CBetaBinomial(alpha=600, beta=400, n=10, m=1).plot(k=k, color='blue', ax=ax, **kwargs)
    ax.legend()
    plt.show()
...
def plot_wikipedia_cdfs():
    ...
    """
    ax = new_axes(width=10, height=10)
    CBetaBinomial(alpha=0.2, beta=0.25, n=10, m=1).cdf().plot(k=k, color='black', ax=ax)
    CBetaBinomial(alpha=0.7, beta=2, n=10, m=1).cdf().plot(k=k, color='red', ax=ax)
    CBetaBinomial(alpha=2, beta=2, n=10, m=1).cdf().plot(k=k, color='green', ax=ax)
    CBetaBinomial(alpha=600, beta=400, n=10, m=1).cdf().plot(k=k, color='blue', ax=ax)
    ax.legend()
    plt.show()
...
def plot_ml_app():
    ...
    bb_1 = CBetaBinomial(n=20, alpha=2, beta=2, m=1)
    ...
    ax[0].legend()
    bb_2 = CBetaBinomial(n=20, alpha=5, beta=2, m=1)
